...
$versions = Get-ChildItem (Join-Path $env:ProgramFiles 'Microsoft Dynamics 365 Business Central\*\Service\Microsoft.Dynamics.Nav.Management.psm1') | Select-Object -Property FullName | Split-Path -Parent | Split-Path -Parent | Split-Path -Leaf
foreach ($version in $versions) {
    Write-Host "Stopping Services for version $($version.Substring(0,$version.Length - 1))"
    $setupParameters.navServicePath  = (Join-Path (Join-Path (Join-Path $env:ProgramFiles 'Microsoft Dynamics 365 Business Central') $version) 'service')
    $setupParameters.navIdePath = (Join-Path (Join-Path (Join-Path ${env:ProgramFiles(x86)} 'Microsoft Dynamics 365 Business Central') $version) 'RoleTailored Client')
    Load-InstanceAdminTools -setupParameters $setupParameters 

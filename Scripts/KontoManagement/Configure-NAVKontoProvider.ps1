        ...
        switch ($input) {
            ...
            default {
                ...
                if ($selectedAccountant) { 
                    ...
                    switch ($input) {
                        '0' { break }
                        '1' { $response = Post-NAVKontoResponse -Provider $Provider -Query "update-bookkeeper?guid=$($selectedAccountant.guid)&status=Active&count_of_bookkeepers=$($selectedAccountant.NoOfUsers)" -Content "{}"  }
